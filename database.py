...
def get_all_products():
    with closing(sqlite3.connect('Ozon_products.db')) as conn:
        ...
        SELECT
            ...
            p.offer_id,
            p.name,
            p.price,                    -- Ozon цена (базовая)
            p.old_price,
            COALESCE(b.admin_price, p.btb_price) AS btb_price,   -- BTB-цена
            p.currency_code,
            p.is_archived,
        ...
        return [{
            ...
            "offer_id": row[2],
            "name": row[3],
            "ozon_price": row[4],        # текущая цена Ozon
            "old_price": row[5],          # старая цена (зачёркнутая)
            "btb_price": row[6],          # BTB-цена или None
            "price": row[6] if row[6] is not None else row[4],  # <-- добавляем поле price!
            "currency_code": row[7],
            "is_archived": row[8],
            "images": row[9].split(', ') if row[9] else [],
            "stocks": row[10].split(', ') if row[10] else []
        } for row in products]
    ...
    try:
        with closing(sqlite3.connect('Ozon_products.db')) as conn:
            ...
            for product in products:
                ...
                # Удаление старых остатков и изображений...
